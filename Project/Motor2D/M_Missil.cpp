			...
			if (it->target->active && it->target->GetHP() > 0)
			{
				...
				if (it->pos.DistanceManhattan(target) < it->vel * dt)
				{
					...
					else
					{
						...
						{
							App->explosion->AddExplosion({ (int)it->target->GetPosition().x, (int)it->target->GetPosition().y }, 30, it->dmg / 3, 0.02f, 1, COMPUTER, EXPLOSION_REAVER, false);
							App->explosion->AddExplosion({ (int)it->target->GetPosition().x, (int)it->target->GetPosition().y }, 50, (it->dmg/3)*2, 0.02f, 1, COMPUTER, EXPLOSION_NONE, false);
						}
					}
