...
void Unit::Attack()
{
	if ((attackingUnit && attackingUnit->state != STATE_DIE) || attackingBuilding && attackingBuilding->state != BS_DEAD)
	{
		...
		if (stats.type == DRAGOON || stats.type == HYDRALISK || stats.type == MUTALISK || stats.type == REAVER || stats.type == SCOUT || stats.type == HIGH_TEMPLAR || stats.type == SCOUT_CIN)
		{
			...
			{
				if (attackingUnit && attackingUnit->movementType == FLYING)
...
void Unit::StartDeath()
{
	...
	{
		App->explosion->AddExplosion({ (int)position.x, (int)position.y }, 40, stats.attackDmg/2, 0.1f, 1, PLAYER, EXPLOSION_TERRAN, false);
		App->explosion->AddExplosion({ (int)position.x, (int)position.y }, 65, stats.attackDmg/2, 0.1f, 1, PLAYER, EXPLOSION_NONE, false);
	}
	if (selected)
	{
...
void Unit::SetGathering(Resource* resource)
{
	if (resource)
	{
		...
		secondGatherSound = thirdGatherSound = false;
		attackState = ATTACK_STAND;
		if (gatheredAmount)
		{
			ReturnResource();
